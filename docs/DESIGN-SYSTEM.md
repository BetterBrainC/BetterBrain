...
**Role themes (per client request):** the primary/CTA button family is themed per signed-in role via a class on the zone layout root, overriding `--primary/-600/-700/-300`, `--border-focus`, `--shadow-focus`, `--shadow-primary` (see `app/globals.css`): `.theme-employee` = OOCA blue `#1D6FE8` (client-requested for the นักกิจกรรมบำบัด zone; AA-adjusted from the OOCA reference), `.theme-admin` + `.theme-director` = OOCA teal `#0fb5a1` / hover `#0a8c7c`, the exact family from the client's reference build (kpcrmv4/TherapistPracticeManagement `src/styles/tokens.css`) — kept as two classes so the roles can re-split later. NOTE: white on `#0fb5a1` is ~2.6:1 (below AA), accepted per client preference to match the old app. Public booking + relatives portal keep the `:root` default. All fills keep white text ≥ 4.5:1 (WCAG AA). Everything reading the `--primary` tokens (buttons, tonal chips, focus rings, and the active nav item in every shell — bottom nav, top nav, sidebar, staff mobile nav) follows automatically — never hard-code a role color on a component. (`--teal` is no longer the nav-active accent; it remains for success/positive touches.)
