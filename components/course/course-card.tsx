...
/**
 * Course progress + lifecycle (P7). At 0 remaining → "ครบคอร์ส" → Continue or
 * No service (→ post-treatment Score). Demo: choice is local state.
 */
    ...
    <Card className="space-y-3">
      ...
      {complete && !decision && (
        <div className="space-y-2 rounded-md bg-surface-tint p-3">
          ...
          <div className="flex gap-2">
            <Button size="sm" className="flex-1" onClick={() => setDecision("continue")}>
              ต่อคอร์ส (Continue)
            </Button>
            <Button
              size="sm"
              variant="secondary"
              className="flex-1"
              onClick={() => setDecision("no_service")}
            >
              ไม่ต่อ (No service)
            </Button>
          </div>
      ...
      {decision === "continue" && (
        <p className="rounded-md bg-[var(--info-bg)] px-3 py-2 text-sm text-[var(--info-fg)]">
          เลือกต่อคอร์ส → กลับสู่กระบวนการเดิม (รอ Admin เพิ่มคอร์ส/คิว)
        </p>
      )}
      {decision === "no_service" && (
        <p className="rounded-md bg-[var(--surface-sunken)] px-3 py-2 text-sm text-muted">
          เลือกไม่ต่อ → พนักงานทำ Summary report + ประเมิน Score หลังฝึก → สถานะ No service
        </p>
      )}
    </Card>
  );
